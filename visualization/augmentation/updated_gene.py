...
#################### sampling strategy
        try:
            if generator_button:
                ...
                with st.spinner('Wait for it...'):
                    ...
                    # # 기본값 처리: feature_selection이 없거나 다른 경우
                    # if not feature_selection:
                    #     # feature_selection이 없는 경우 자동으로 Multiclass 처리
                    #     if data_for_labelencoding and label_col_name:
                    #         unique_labels = updated_df[label_col_name].unique()
                    #         value_counts = updated_df[label_col_name].value_counts()
                    ...
                    #         print(f"자동 Multiclass sampling_strategy (인코딩된): {sampling_strategy}")
                    #         print(f"증강 비율: {sampling_threshold}%")
                    #         print(f"원본 카운트: {dict(value_counts)}")
                    #     else:
                    #         unique_labels = updated_df[target_feature[0]].unique()
                    #         value_counts = updated_df[target_feature[0]].value_counts()
                    ...
                    #         print(f"자동 Multiclass sampling_strategy (원본): {sampling_strategy}")
                    #         print(f"증강 비율: {sampling_threshold}%")
                    #         print(f"원본 카운트: {dict(value_counts)}")
    ...
    #################### Oversampling Start
                    ...
                    # X = updated_df.drop(target_feature, axis=1) 
                    # y = updated_df[target_feature]
                    # target_col = target_feature[0] if isinstance(target_feature, list) else target_feature
                    # X = updated_df.drop(target_col, axis=1) 
                    # y = updated_df[target_col]
                    # print("====================")
                    # print(y)
