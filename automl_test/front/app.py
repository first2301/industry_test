...
with st.spinner('Wait for it...'):
    ...
    if uploaded_file is not None or df is not None:
        ...
        target_feture = ""
        if option == '분류' or option == '회귀' or option == '군집':
        ...
        if button_for_training: # 분류, 이상탐지 옵션에 따라 머신러닝 학습 진행
            ...
            if option == '분류' or option == '군집' or option == '회귀':
                ...
                with st.spinner('Wait for it...'):
                    ...
                    if option == '분류':
                        response = requests.post('http://127.0.0.1:8000/automl/classification', json=data)
                    elif option == '군집':
                        response = requests.post('http://127.0.0.1:8000/automl/clustering', json=data)
                    elif option == '회귀':
                        response = requests.post('http://127.0.0.1:8000/automl/regression', json=data)
                    if response.status_code == 200: 
                        ...
                        if option == '분류':
                            ...
                            clf_dfs = []
                            for i in data:
                                best_json = data[i]["best"]
                                best_df = pd.read_json(StringIO(best_json))
                                clf_dfs.append(best_df)
                            clf_df = pd.concat(clf_dfs, axis=1)
                            sorted_df = clf_df.sort_values(by='f1_weighted', ascending=False)
                        if option == '회귀':
                            # JSON 문자열을 DataFrame으로 변환
                            reg_dfs = []
                            for i in data:
                                best_json = data[i]["best"]
                                best_df = pd.read_json(StringIO(best_json))
                                reg_dfs.append(best_df)
                            reg_df = pd.concat(reg_dfs, axis=1)
                            sorted_df = reg_df.sort_values(by='neg_mean_squared_error', ascending=False)
                        if option == '군집':
                            # JSON 문자열을 DataFrame으로 변환
                            cluster_dfs = []
                            for i in data:
                                best_json = data[i]["best"]
                                best_df = pd.read_json(StringIO(best_json))
                                cluster_dfs.append(best_df)
                            cluster_df = pd.concat(cluster_dfs, axis=1)
                            sorted_df = cluster_df.sort_values(by='silhouette_score', ascending=False)
                        # sorted_concat_df = concat_df.sort_values(by='f1_weighted', ascending=False)

                        # 모델 추천
                        st.title('Augmented Analysis')
                        st.line_chart(sorted_df)
